...
function setupExpensePage9x() {
    var _el_u_exp9x_form = document.getElementById('u_exp9x_form');
    if (_el_u_exp9x_form) _el_u_exp9x_form.addEventListener('submit', expRq9x_submitExpense);
    var _el_u_exp9x_fetch_button = document.getElementById('u_exp9x_fetch_button');
    if (_el_u_exp9x_fetch_button) _el_u_exp9x_fetch_button.addEventListener('click', expRq9x_fetchExpenses);
}
        ...
        if (status === 200) {
            ...
            expenses.forEach(function(expense) {
                ...
                tr.innerHTML = `
                    ...
                    <td><button onclick="expRq9x_deleteExpense('${expense.id}')">🗑️</button></td>
                `;
...
function setupMaintenancePage5h() {
    var _el_mntRq5h_formSubmit = document.getElementById('mntRq5h_formSubmit');
    if (_el_mntRq5h_formSubmit) _el_mntRq5h_formSubmit.addEventListener('submit', mntRq5h_submitMaintenance);
}
...
function setupStockEditPagev9t() {
    var _el_stkEd_v9t_formSubmit = document.getElementById('stkEd_v9t_formSubmit');
    if (_el_stkEd_v9t_formSubmit) _el_stkEd_v9t_formSubmit.addEventListener('submit', stkEdv9t_submitStockEdit);
}
    ...
    sendRequest9x('GET', '/api/admin/fetch-pending', null, function(status, data) {
        if (status === 200 && data) {
            ...
            data.forEach(function(req) {
                ...
                // Create the main info block safely
                div.innerHTML = `
                    <p><strong>Type:</strong> ${req.type} | <strong>ID:</strong> ${req.id}</p>
                    <p><strong>Details:</strong> ${req.details}</p>
                    <p><strong>Reason:</strong> ${req.reason || '-'}</p>
                    <p><strong>Amount:</strong> ${req.amount !== null ? parseFloat(req.amount).toFixed(2) + ' OMR' : '-'}</p>
        ...
        if (status === 200 && data) {
            ...
            data.forEach(function(req) {
                ...
                tr.innerHTML = `
                    ...
                    <td>${req.reason}</td>
                    <td>${req.status}</td>
                    <td><button onclick="deleteMaintenanceRequest('${req.id}')">🗑️</button></td>
                `;
                tbody.appendChild(tr);
            });
        } else {
        ...
        if (status === 200 && data) {
            ...
            data.forEach(function(req) {
                ...
                tr.innerHTML = `
                    ...
                    <td>${req.reason}</td>
                    <td>${req.status}</td>
                    <td><button onclick="deleteStockEditRequest('${req.id}')">🗑️</button></td>
                `;
                tbody.appendChild(tr);
            });
        } else {
...
function deleteStockEditRequest(id) {
    ...
    sendRequest9x('DELETE', '/api/delete-stockedit/' + id, null, function(status) {
        if (status === 200) {
            alert('Stock edit request deleted successfully.');
...
document.getElementById('u_exp9x_print').addEventListener('click', function () {
    // Remove previous print area if any
    let oldPrint = document.querySelector('.print-area');
    if (oldPrint) oldPrint.remove();

    // Clone data from user info and expenses table
    const userName = document.getElementById('u_exp9x_name').value || document.getElementById('pharmacist-name').textContent;
    const department = document.getElementById('u_exp9x_department').value || '';
    const period = document.getElementById('u_exp9x_period').value || '';
    const total = document.getElementById('u_exp9x_total').textContent;

    // Get year & month for print heading
    const year = document.getElementById('u_exp9x_view_year').value;
    const month = document.getElementById('u_exp9x_view_month').selectedOptions[0]?.text || '';

    // Clone the expenses table data
    const table = document.getElementById('u_exp9x_expenses_table').cloneNode(true);

    // Remove the "Actions" column for printing
    const actionColIndex = Array.from(table.rows[0].cells).findIndex(cell => cell.textContent === "Actions");
    if (actionColIndex !== -1) {
    ...
    printDiv.innerHTML = `
      <div class="print-letterhead">
        ...
        <div class="print-company-details">
            <div>Al Salam Retail Pharmacy</div>
            <div>Expense Request Report</div>
            <div>Period: <b>${month} ${year}</b></div>
        </div>
      </div>
      <div class="print-title">Expense Requests Summary</div>
      <div>
        <b>Employee:</b> ${userName} &nbsp; &nbsp; 
        <b>Department:</b> ${department} &nbsp; &nbsp; 
        <b>Period:</b> ${period || (month + " " + year)}
      </div>
    `;
    printDiv.appendChild(table);

    printDiv.innerHTML += `
      <div class="print-title" style="font-size:1.15em; margin-top:24px;">Total Approved: <b>${total} OMR</b></div>
      <div class="print-footer">
        Printed on: ${new Date().toLocaleString()} <br>
        Designed by Pharmacist: MOHAMED HAMID
      </div>
    `;
    ...
    // Remove print area after print (to keep DOM clean)
    setTimeout(() => {
        if (printDiv) printDiv.remove();
    }, 500);
});

document.getElementById('u_exp9x_export_excel').addEventListener('click', function () {
    const year = document.getElementById('u_exp9x_view_year').value;
    const month = document.getElementById('u_exp9x_view_month').value;
    if (!year || !month) {
        alert('Please select year and month!');
    ...
    fetch('/api/expense/export-excel', {
        method: 'POST',
    ...
    .then(res => {
        if (!res.ok) throw new Error('No data to export or server error');
        return res.blob();
    })
    .then(blob => {
